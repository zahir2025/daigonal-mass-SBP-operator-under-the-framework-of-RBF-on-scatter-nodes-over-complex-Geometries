"""
Phase 12 - Nonlinear Burgers With Path A Entropy-Conservative SBP
=================================================================

This phase follows the same construction trend as the earlier work:

    1. Generate nodes on the selected physical domain.
...
Outputs:
    outputs_phase12_nonlinear_burgers/
        phase12_pathA_scheme_geometry_<domain>.png
        phase12_pathA_burgers_<domain>_raw.csv
        phase12_pathA_burgers_<domain>_summary.csv
        phase12_pathA_burgers_<domain>_error_vs_N.png
        phase12_pathA_burgers_<domain>_error_vs_h.png
        phase12_pathA_burgers_<domain>_observed_orders.png
        phase12_pathA_burgers_<domain>_field_error.png
"""
...
NODE_REFINEMENTS_BY_DOMAIN = {
    "annulus": [400, 800, 1600, 3200, 6400],
    "box_minus_circle": [400, 800, 1600, 3200, 6400],
    "box_minus_airfoil": [400, 800, 1600, 3200, 6400],
}
...
OUTPUT_DIR = SCRIPT_DIR / "outputs_phase12_nonlinear_burgers"
...
def plot_scheme_geometry(domain_type, domain):
    ...
    points = steady.generate_nodes(domain_type, domain, N_plot)
    cells = steady.clipped_voronoi(points, domain)
    weights = steady.lumped_mass_matrix(cells)
    ...
    axes[0].set_title("Physical cloud nodes", fontweight="bold")
    ...
    fig.suptitle(
        f"Phase 12 Path A nonlinear Burgers geometry: {domain_type}",
        fontsize=14,
        fontweight="bold",
    )

    path = OUTPUT_DIR / f"phase12_pathA_scheme_geometry_{domain_type}.png"
...
def plot_error_vs_N(rows, domain_type):
    ...
    fig.tight_layout()
    path = OUTPUT_DIR / f"phase12_pathA_burgers_{domain_type}_error_vs_N.png"
    path = save_figure(fig, path)
    plt.close(fig)
    return path
...
def plot_error_vs_h(rows, domain_type):
    ...
    fig.tight_layout()
    path = OUTPUT_DIR / f"phase12_pathA_burgers_{domain_type}_error_vs_h.png"
    path = save_figure(fig, path)
    plt.close(fig)
    return path
...
def plot_observed_orders(summary, domain_type):
    ...
    fig.tight_layout()
    path = OUTPUT_DIR / f"phase12_pathA_burgers_{domain_type}_observed_orders.png"
    path = save_figure(fig, path)
    plt.close(fig)
    return path
...
def plot_field_error(domain_type, domain):
    ...
    path = OUTPUT_DIR / f"phase12_pathA_burgers_{domain_type}_field_error.png"
    path = save_figure(fig, path)
    plt.close(fig)
    return path, rel_err
...
def main():
    ...
    print("=" * 94)
    print("PHASE 12 - NONLINEAR BURGERS WITH PATH A ENTROPY-CONSERVATIVE SBP")
    print("=" * 94)
    ...
    print(f"N values     : {node_refinements}")
    ...
    raw_csv = write_csv(
        OUTPUT_DIR / f"phase12_pathA_burgers_{domain_type}_raw.csv",
        rows,
        RAW_FIELDS,
    )
    summary_csv = write_csv(
        OUTPUT_DIR / f"phase12_pathA_burgers_{domain_type}_summary.csv",
        summary,
        SUMMARY_FIELDS,
    )
    ...
    print("\n" + "=" * 94)
    print("PHASE 12 PATH A ENTROPY-CONSERVATIVE NONLINEAR BURGERS COMPLETE")
    print("=" * 94)
    ...
    print("-" * 94)
    print("p | levels | finest N | flux err | order | last | max SBP | mass bal | entropy bal")
    print("-" * 94)
