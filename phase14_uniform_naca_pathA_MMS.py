"""
Phase 14 - Uniform NACA Airfoil Domain With The Current Path A SBP-SAT Scheme
============================================================================
...
The only intended change from the scattered-node airfoil tests is the node
distribution:

    scattered cloud nodes -> Cartesian uniform nodes clipped to the NACA domain.

The physical domain is the existing project domain "box_minus_airfoil":

    [-1,2] x [-1,1] minus the NACA0012 airfoil.
...
Outputs:
    outputs_phase14_uniform_naca/
        phase14_uniform_naca_raw.csv
        phase14_uniform_naca_summary.csv
        phase14_uniform_naca_geometry.png
        phase14_uniform_naca_error_vs_N.png
        phase14_uniform_naca_error_vs_h.png
        phase14_uniform_naca_observed_orders.png
        phase14_uniform_naca_diagnostics.png
        phase14_uniform_naca_summary_table.png
        phase14_uniform_naca_field_error.png
"""
...
DOMAIN_TYPE = "box_minus_airfoil_uniform"
BASE_DOMAIN_TYPE = "box_minus_airfoil"
...
FIELD_P = 5
FIELD_GRID_SHAPE = (48, 32)
GEOMETRY_GRID_SHAPE = (24, 16)

FIG_DPI = 190

SCRIPT_DIR = Path(__file__).resolve().parent
OUTPUT_DIR = SCRIPT_DIR / "outputs_phase14_uniform_naca"
...
def uniform_nodes_in_domain(domain, nx, ny):
    ...
    if abs(hx - hy) > 1.0e-14:
        raise RuntimeError(f"grid spacing is not uniform: hx={hx}, hy={hy}")

    xs = minx + (np.arange(nx, dtype=float) + 0.5) * hx
    ys = miny + (np.arange(ny, dtype=float) + 0.5) * hy
...
def run_one_case(domain, p, nx, ny, keep_fields=False):
    ...
    print("-" * 90)
    print(
        f"NACA uniform, p={p}, grid={nx}x{ny}, N={N}, "
        f"h={h:.6e}, Nc={Nc}"
    )
...
def plot_uniform_geometry(domain):
    ...
    axes[0].set_title("Uniform nodes clipped to NACA domain", fontweight="bold")
    ...
    fig.suptitle(
        f"Phase 14 geometry: NACA airfoil domain, uniform {nx}x{ny} background grid",
        fontsize=13,
        fontweight="bold",
    )

    path = OUTPUT_DIR / "phase14_uniform_naca_geometry.png"
...
def plot_error_vs_N(rows):
    ...
    ax.set_title("Uniform NACA steady MMS error vs N")
    ax.grid(True, which="both", alpha=0.30)
    ax.legend(ncol=2, fontsize=8)

    fig.tight_layout()
    path = OUTPUT_DIR / "phase14_uniform_naca_error_vs_N.png"
    path = save_figure(fig, path)
    plt.close(fig)
...
def plot_error_vs_h(rows):
    ...
    ax.set_ylabel("relative M_L solution error")
    ax.set_title("Uniform NACA steady MMS error vs h")
    ax.grid(True, which="both", alpha=0.30)
    ax.legend(ncol=2, fontsize=8)

    fig.tight_layout()
    path = OUTPUT_DIR / "phase14_uniform_naca_error_vs_h.png"
    path = save_figure(fig, path)
    plt.close(fig)
...
def plot_observed_orders(summary):
    ...
    ax.set_title("Observed uniform-NACA convergence orders")
    ax.set_xticks(p)
    ax.grid(True, alpha=0.30)
    ax.legend(fontsize=8)

    fig.tight_layout()
    path = OUTPUT_DIR / "phase14_uniform_naca_observed_orders.png"
    path = save_figure(fig, path)
    plt.close(fig)
...
def plot_diagnostics(summary):
    ...
    fig.suptitle("Phase 14 uniform-NACA scheme diagnostics", fontsize=13, fontweight="bold")

    path = OUTPUT_DIR / "phase14_uniform_naca_diagnostics.png"
    path = save_figure(fig, path)
    plt.close(fig)
...
def plot_summary_table(summary):
    ...
    ax.set_title(
        "Phase 14 uniform NACA MMS error display",
        fontsize=14,
        fontweight="bold",
        pad=14,
    )

    path = OUTPUT_DIR / "phase14_uniform_naca_summary_table.png"
    path = save_figure(fig, path)
    plt.close(fig)
...
def plot_field_error(domain, fields):
    ...
    fig.suptitle(
        (
            "Uniform NACA steady advection MMS "
    ...
    path = OUTPUT_DIR / "phase14_uniform_naca_field_error.png"
    path = save_figure(fig, path)
    plt.close(fig)
...
def main():
    ...
    print("=" * 90)
    print("PHASE 14 - UNIFORM NACA AIRFOIL DOMAIN WITH CURRENT PATH A SBP-SAT")
    print("=" * 90)
    print("domain              = [-1,2] x [-1,1] minus NACA0012")
    ...
    raw_csv = write_csv(
        OUTPUT_DIR / "phase14_uniform_naca_raw.csv",
        rows,
        RAW_FIELDS,
    )
    summary_csv = write_csv(
        OUTPUT_DIR / "phase14_uniform_naca_summary.csv",
        summary,
        SUMMARY_FIELDS,
    )
    ...
    print("\n" + "=" * 90)
    print("PHASE 14 UNIFORM NACA RUN COMPLETE")
    print("=" * 90)
